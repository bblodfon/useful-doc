...
DNA => RNA => protein synthesis (one way)

#### Exon

After the transcription process, the mRNA is a series of nucleotides that 
includes regions that code for a protein (**exons**) and non-coding regions 
that are called **introns**. The **introns** are removed by *RNA splicing*. 

#### Codon

A **three-base** sequence of DNA that **encodes a single amino acid** in the 
genetic code. There are in total 64 codons. 4 of them are special: one codes 
for "start" and three for "stop".

#### Gene Expression

**Gene expression** is the process that synthesizes a protein out of an RNA 
strand. It is divided into 2 parts: **transcription** (happens inside the 
nucleus) and **translation** (happens in the cytoplasm of the cell). Watch the 
[video](https://www.youtube.com/watch?v=2BwWavExcFI) of the whole process as 
described below:

At **transcription**, the DNA in the gene is used as a template to make a 
**messenger RNA strand (mRNA)** with the help of the **enzyme RNA polymerase**. 
- Initiation stage: the RNA polymerase enzyme recognizes the **promoter region** 
of a gene and binds there (the DNA helix is unwind and opened thus) 
- Elongation stage: the RNA polymerase enzyme slides down the template DNA 
strand and the complimentary bases pair up (**template polymerization**)
- Termination stage: the **terminator region** of the gene is reached and the DNA 
strand, the RNA polymerase and the mRNA dissociate from each other

Before the mRNA leaves the nucleus, some modifications are done first to produce 
the mature mRNA:
- **intros** (non-coding regions) are removed (RNA splicing)
- a 5' cap and a 3' poly-A tail are added (mRNA capping for directionality and 
protection - start 5', end 3')

At **translation**, the mRNA nuclueotides (the coding region) will be used to 
synthesize the protein. The process is as follows:
- During initiation, the mRNA strand binds to a **small ribosomal subunit** 
upstream of the start codon
- Each aminod acid is brought by a specific transfer RNA molecule (tRNA) which
has an anticodon that uniquely identifies a codon and the corresponding amino 
acid
- After the initiator tRNA binds to the start codon, the **large ribosomal 
subunit** binds to form the translation complex. There are 3 distinct regions 
in the large ribosomal subunit: **E, P, A**. The initiator tRNA binds to the 
**P** region
- During elongation, a charged tRNA with an amino acid is brought to the **A** 
region and binds to the specified codon through **templated polymerization**
- Peptide bond forms between the two amino acids of the **P** and **A** regions
- The complex slides down one codon to the right and the uncharged tRNA of the 
**E** region exits
- **A** region opens to accept the next charged tRNA
- This elongation procedure continues until a stop codon is reached
- Then, a release factor binds to the **A** site at the stop codon and the 
**polypeptide** is released from the tRNA in the **P** region
- Lastly the whole complex dissociates

The produced polypeptide (protein) may need to be further modified before its 
ready to function (PTM).
...
A **transcription terminator** is a section of nucleic acid sequence that marks 
the end of a gene in genomic DNA during transcription. This sequence mediates 
transcriptional termination by providing signals in the newly synthesized 
transcript RNA that trigger processes which release the transcript RNA from 
the transcriptional complex. Release of the transcriptional complex frees RNA 
...
A homologous gene (or **homolog**) is a gene inherited in two species by a 
common ancestor.

#### Protein Dimer

Portein dimer is a macromolecular complex formed by two protein monomers 
(single proteins).  
...
Examples: proteomics, metabolomics, genomics, transcriptomics, proteome, 
metabolome, genome, transcriptome.        
...
NF-kB is a protein complex that controls transcription of DNA, cytokine 
production and cell survival and it plays a key role in regulating the immune 
response to infection.

- NF-kB signaling in the innate Immune system
- **dysregulation of NFkB can lead to disease (inflammatory disease, cancer)**, 
...
- activation of NF-kB is done by a wide range of signals
  ...
  - through the TLR/IL-1R superfamily
- purine, pyrimidine, nucleotide

#### IRF/JAK-STAT Signaling

#### Cytokines                                                       
                                                                                
A broad and loose category of small proteins used in cell signaling. Cytokines 
